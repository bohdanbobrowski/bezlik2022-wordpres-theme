 ...
 * Enqueue assets
 */
function bezlik_enqueue_gutenberg_assets() {

	wp_enqueue_style( 'bezlik-block-editor-styles', get_template_directory_uri() . '/assets/css/editor.min.css', array(), BEZLIK_VERSION );
	...
	foreach ( $background_colors as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_css( $selector, 'background-color', $theme_mod );
	}
	...
	foreach ( $colors as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_css( $selector, 'color', $theme_mod );
	}
	...
	foreach ( $border_colors as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_css( $selector, 'border-color', $theme_mod );
	}
	...
	foreach ( $border_radius as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_css( $selector, 'border-radius', $theme_mod, '', 'px' );
	}
	...
	*/
	...
	foreach ( $font_sizes as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_responsive_css( $selector, 'font-size', $theme_mod, '', 'px' );
	}			
	...
	foreach ( $paddings_tb as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_responsive_css( $selector, 'padding-top', $theme_mod, '', 'px' );
		$css .= Bezlik_Custom_CSS::generate_responsive_css( $selector, 'padding-bottom', $theme_mod, '', 'px' );
	}	
	...
	foreach ( $paddings_lr as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_responsive_css( $selector, 'padding-left', $theme_mod, '', 'px' );
		$css .= Bezlik_Custom_CSS::generate_responsive_css( $selector, 'padding-right', $theme_mod, '', 'px' );
	}			
	...
	foreach ( $text_align as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_css( $selector, 'text-align', $theme_mod );
	}	
	...
	foreach ( $text_transform as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_css( $selector, 'text-transform', $theme_mod );
	}	
	...
	foreach ( $letter_spacing as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_css( $selector, 'letter-spacing', $theme_mod, '', 'px' );
	}	
	...
	foreach ( $line_height as $theme_mod => $selector ) {
		$css .= Bezlik_Custom_CSS::generate_css( $selector, 'line-height', $theme_mod );
	}		
	...
	wp_add_inline_style( 'bezlik-block-editor-styles', $css );	

}
add_action( 'enqueue_block_editor_assets', 'bezlik_enqueue_gutenberg_assets' );