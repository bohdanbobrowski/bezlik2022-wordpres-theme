...
if ( !class_exists( 'Bezlik_Footer' ) ) :
	...
	Class Bezlik_Footer {
		...
		public function __construct() {		
			add_action( 'bezlik_footer_before', array( $this, 'sidebar_cart' ) );
			add_action( 'bezlik_footer', array( $this, 'before_footer' ) );
			add_action( 'bezlik_footer', array( $this, 'template' ) );
		}
		...
		public function sidebar_cart() {
			if ( class_exists( 'Woocommerce' ) ) : ?>
				<div id="sidebar-cart" class="sidebar-cart">
					<div tabindex="0" class="sidebar-cart-close"><?php bezlik_get_svg_icon( 'icon-cancel', true ); ?></div>
					<span>
						<?php
						$instance = array(
							'title' => esc_html__( 'Your cart', 'bezlik' ),
						);
		...
		public function before_footer_cta() {
			$title 			= get_theme_mod( 'before_footer_cta_title', esc_html__( 'Have something on your mind?', 'bezlik' ) );
			$button_link 	= get_theme_mod( 'before_footer_cta_link', '#' );
			$button_text 	= get_theme_mod( 'before_footer_cta_text', esc_html__( 'Say hello', 'bezlik' ) );
		...
		public function before_footer_social() {
			...
			foreach ( $socials as $social ) {
				...
				if ( $network ) {
					$items .= '<a target="_blank" rel="noopener noreferrer nofollow" href="' . esc_url( $social ) . '">' . bezlik_get_svg_icon( 'icon-' . esc_html( $network ), false ) . ucwords( esc_html( $network ) ) . '</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			}
			$items .= '</div>';
		...
		public function template() {
			...
			$footer_credits = get_theme_mod( 'footer_credits', sprintf( esc_html__( 'Powered by the %1$1s WordPress theme', 'bezlik' ), '<a rel="nofollow" href="https://elfwp.com/themes/bezlik">Elsie</a>' ) );
