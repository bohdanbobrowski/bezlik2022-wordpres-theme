...
if ( !class_exists( 'Bezlik_Header' ) ) :
	...
	Class Bezlik_Header {
		...
		public function __construct() {
			add_action( 'bezlik_header', array( $this, 'header_markup' ) );
		}
		...
		public function header_markup() {
			...
			if ( isset( $post ) ) {
				$disable_header	= get_post_meta( $post->ID, '_bezlik_hide_header', true );	
				if ( $disable_header ) {
					return;
				}
			}
		...
		public function header_layout_inline() {
			...
			<header id="masthead" class="site-header menu-bar header-layout-inline">
				<?php do_action( 'bezlik_menu_bar_inside' ); ?>
					...
					<div class="row">
						<div class="col-md-3 col-8 v-align">
							<div class="site-branding">
								...
								$bezlik_description = get_bloginfo( 'description', 'display' );
								if ( $bezlik_description || is_customize_preview() ) :
									?>
									<p class="site-description"><?php echo $bezlik_description; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
								<?php endif; ?>
							</div><!-- .site-branding -->
						</div>
					...
					<div class="row">
						...
						<div class="col-5 col-md-8 v-align">
							...
							<div class="header-elements v-align align-right">
								...
								<?php $this->header_woocommerce(); ?>
								<?php $this->header_button(); ?>
								<button class="menu-toggle" aria-controls="primary-menu" aria-label="<?php echo esc_attr__( 'Toggle mobile menu', 'bezlik' ); ?>" aria-expanded="false" <?php echo wp_kses_post( apply_filters( 'bezlik_nav_toggle_data_attrs', '' ) ); ?>><?php bezlik_get_svg_icon( 'icon-bars', true ); ?><span class="menu-label"><?php echo esc_html( $mobile_label ); ?></span></button>	
							</div>									
						</div>			
					</div>
				</div>
		...
		public function topbar_text() {
			$text = get_theme_mod( 'top_bar_text', esc_html__( 'Your custom text', 'bezlik' ) );
		...
		public function main_navigation( $mobile = false ) {
			...
			<?php else: ?>	
				<nav id="<?php echo esc_attr( $nav_id ); ?>" class="main-navigation" <?php echo wp_kses_post( apply_filters( 'bezlik_nav_data_attrs', '' ) ); ?>>
					<div class="mobile-menu-close" tabindex="0"><?php bezlik_get_svg_icon( 'icon-cancel', true ); ?></div>
					<?php
					wp_nav_menu( array(
		...
		public function header_button() {
			...
			$text 	= get_theme_mod( 'header_button_text', esc_html__( 'Click here', 'bezlik' ) );
		...
		public function header_woocommerce() {
			...
			?>
			<?php echo bezlik_woocommerce_header_cart(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<?php
		}
		...
		public function topbar_social() {
			...
			foreach ( $socials as $social ) {
				$network = bezlik_get_social_network( $social );
				if ( $network ) {
					$items .= '<a target="_blank" rel="noopener noreferrer nofollow" href="' . esc_url( $social ) . '">' . bezlik_get_svg_icon( 'icon-' . esc_html( $network ), false ) . '</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			}
			$items .= '</div>';
		...
		public function header_search( $mobile = false ) {
			...
			echo 	'<span tabindex="0" class="header-search-toggle">' . bezlik_get_svg_icon( 'icon-search', false ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo 	'<span tabindex="0" class="header-search-cancel hide">' . bezlik_get_svg_icon( 'icon-cancel', false ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '</div>';
		}
		...
		public function topbar_contact() {
			...
			$phone 	= get_theme_mod( 'top_bar_contact_phone', esc_html__( '+999.999.999', 'bezlik' ) );
			$email 	= get_theme_mod( 'top_bar_contact_email', esc_html__( 'office@example.org', 'bezlik' ) );
			?>

			<div class="top-bar-contact">
				<?php if ( $email ) : ?>
					<a href="mailto:<?php echo esc_attr( antispambot( $email ) ); ?>"><?php bezlik_get_svg_icon( 'icon-mail', true ); ?><?php echo esc_html( antispambot( $email ) ); ?></a>
				<?php endif; ?>
				<?php if ( $phone ) : ?>
					<a href="tel:<?php echo esc_attr( $phone ); ?>"><?php bezlik_get_svg_icon( 'icon-phone', true ); ?><?php echo esc_html( $phone ); ?></a>
				<?php endif; ?>					
			</div>			
