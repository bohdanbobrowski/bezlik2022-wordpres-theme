...
class Bezlik_Install_Plugins {
	...
	public static function instance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Main ) ) {
			self::$instance = new Bezlik_Install_Plugins();
		}
	...
	public function do_plugin_install(){
		...
		$plugins_title     = array(
			esc_html__( 'Bezlik Companion', 'bezlik' ),
			esc_html__('Starter Sites', 'bezlik')
		);
		...
		foreach ( $not_installed_or_activated_plugins_id as $key => $value ) {
			...
			$links_attrs[$key] = array(
				...
				'data-activating-label' => /* translators: %s: plugin name */ sprintf( __( 'Activating %s', 'bezlik' ), $progress_text_and_title ),
				'data-installing-label' => /* translators: %s: plugin name */ sprintf( __( 'Installing %s', 'bezlik' ), $progress_text_and_title ),
				'data-activate-label'   => /* translators: %s: plugin name */ sprintf( __( 'Activate %s'  , 'bezlik' ), $progress_text_and_title ),
				'data-install-label'    => /* translators: %s: plugin name */ sprintf( __( 'Install %s'   , 'bezlik' ), $progress_text_and_title ),
				...
				'data-wpnonce'          => wp_create_nonce( 'bezlik-pi_setup_nonce' )
			);
			...
			if( ! isset( $installed_plugins[ $plugins_base_name[$value] ] ) ){
				...
				$links_attrs[$key]['button_label'] =  /* translators: %s: plugin name */ sprintf( esc_html__( 'Install %s', 'bezlik' ), $progress_text_and_title );
				...
				$links_attrs[$key]['button_label'] =  /* translators: %s: plugin name */ sprintf( esc_html__( 'Activate %s', 'bezlik' ), $progress_text_and_title );
			}
		}

	?>
		<?php if ( empty( $not_installed_or_activated_plugins_id ) ) : ?>
		<div class="bezlik-message bezlik-pi-notice-wrapper bezlik-pi-notice-install-now" style="display:inline-block;">
			<a class="button" href="<?php echo esc_url( admin_url( 'themes.php?page=advanced-import.php' ) ); ?>"><?php echo esc_html__( 'Import a demo', 'bezlik' ); ?></a>
		</div>		
		<?php else : ?>
		<div class="bezlik-message bezlik-pi-notice-wrapper bezlik-pi-notice-install-now" style="display:inline-block;">
			<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<a class="button button-primary bezlik-install-now bezlik-pi-not-installed" data-info='<?php echo wp_json_encode( $links_attrs);?>' ><?php echo $links_attrs[0]['button_label']; ?></a>
		</div>
		<?php endif; ?>
	<?php
	}
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'bezlik-plugin-install', get_template_directory_uri() . '/inc/onboarding/plugin-install.js', array( 'jquery' ), '', true );

		wp_localize_script(
			'bezlik-plugin-install',
			'bezlikPluginInstall',
			array(
				'activating' => esc_html__( 'Activating ', 'bezlik' ),
			)
		);
		...
		wp_enqueue_script( 'bezlik-plugin-install' );

	}
}

$bezlik_plugin_install = new Bezlik_Install_Plugins();