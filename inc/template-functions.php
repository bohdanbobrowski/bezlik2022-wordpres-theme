 ...
 * @return array
 */
function bezlik_body_classes( $classes ) {
	...
	if ( ! is_active_sidebar( 'sidebar-1' ) || ( false === apply_filters( 'bezlik_enable_sidebar', true ) ) ) {
		$classes[] = 'no-sidebar';
	}

	global $post;
	if ( isset( $post) ) {
		$page_layout = get_post_meta( $post->ID, '_bezlik_page_layout', true );	
	...
	//header image class
	$hero_type = get_theme_mod( 'bezlik_hero_type', 'header_image' );
...
add_filter( 'body_class', 'bezlik_body_classes' );
 ...
 * @return array
 */
function bezlik_post_classes( $classes ) {
...
add_filter( 'post_class', 'bezlik_post_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function bezlik_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'bezlik_pingback_header' );

/**
 * Add sidebar
 */
function bezlik_sidebar() { 

	if ( false === apply_filters( 'bezlik_enable_sidebar', true ) ) { 
		return;
	}
	
	get_sidebar();
}
add_action( 'bezlik_sidebar', 'bezlik_sidebar' );

/**
 * Filter the sidebar
 */
function bezlik_filter_sidebar() {
...
add_action( 'wp', 'bezlik_filter_sidebar' );

/**
 * Build fonts URL
 */
function bezlik_generate_fonts_url() {
	$fonts_url = '';
	$subsets = 'latin';
	...
	$body_font		= get_theme_mod( 'bezlik_body_font', $defaults_body );
	$headings_font 	= get_theme_mod( 'bezlik_headings_font', $defaults_headings );
 ...
 */
function bezlik_get_svg_icon( $icon, $echo = false ) {
	$svg_code = wp_kses(
		bezlik_SVG_Icons::get_svg_icon( $icon ),
		array(
			'svg'     => array(
				'class'       => true,
				'xmlns'       => true,
	...
	if ( $echo != false ) {
		echo '<span class="bezlik-icon">' . $svg_code . '</span>'; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	} else {
		return '<span class="bezlik-icon">' . $svg_code . '</span>';
	}
}
 ...
 * Excerpt length
 */
function bezlik_excerpt_length( $length ) {
...
add_filter( 'excerpt_length', 'bezlik_excerpt_length', 999 );

/**
 * Remove archive labels
 */
function bezlik_remove_archive_labels( $title ) {
    if ( is_category() ) {
...
add_filter( 'get_the_archive_title', 'bezlik_remove_archive_labels' );

/**
 * Post types helper
 */
function bezlik_post_types_helper() {
...
function bezlik_get_all_posts_helper() {
	$args = array(
 ...
 */
function bezlik_archive_titles() { 
	...
	<header class="page-header">
		<div class="container">
			<?php
				...
				printf( esc_html__( 'Search Results for: %s', 'bezlik' ), '<h1 class="page-title">' . get_search_query() . '</h1>' );
			?>
		</div>
	</header><!-- .page-header -->
	<?php
...
add_action( 'bezlik_after_header', 'bezlik_archive_titles' );

/**
 * Theme colors
 */
if ( !function_exists( 'bezlik_theme_colors' ) ) {
	function bezlik_theme_colors() {
		return array(
			'color-accent' 				=> '#950b0b',
 ...
 * Sharing options
 */
function bezlik_post_sharing_links() {
	...
	$sharing_title 	= get_theme_mod( 'post_share_title', esc_html__( 'Share this post', 'bezlik' ) );
	...
	$output = 	'<div class="bezlik-sharing">';
	if ( '' !== $sharing_title ) {
	...
	$output .= 		'<div class="sharing-icons">';
						foreach ( $urls as $network => $url ) {
							if ( $url['enabled'] ) {
								$output .= '<a target="_blank" href="' . $url['url'] . '" class="share-button share-button-' . esc_attr( $network ) . '">' . bezlik_get_svg_icon( 'icon-' . $network, false ) . '</a>';
							}
						}
	$output .= 		'</div>';
	$output .= 	'</div>';
 ...
 * Submenu symbols
 */
function bezlik_submenu_symbols( $item_output, $item, $depth, $args ) {
	...
	if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
		return $item_output . '<span tabindex=0 class="icon-dropdown">' . bezlik_get_svg_icon( 'icon-dropdown', false ) . '</span>';
	}

    return $item_output;
}
add_filter( 'walker_nav_menu_start_el', 'bezlik_submenu_symbols', 10, 4 );

/**
 * Masonry init
 */
function bezlik_masonry_init() {
 ...
 * Get social network
 */
function bezlik_get_social_network( $social ) {
 ...
 * Boxes
 */
function bezlik_featured_boxes() {
	...
	if ( ( $front_page && is_home() ) || ( $blog_page && is_front_page() ) ) {
		...
		$title1 	= get_theme_mod( 'featured_box_title1', esc_html__( 'Travel', 'bezlik' ) );
		$title2 	= get_theme_mod( 'featured_box_title2', esc_html__( 'Fashion', 'bezlik' ) );
		$title3 	= get_theme_mod( 'featured_box_title3', esc_html__( 'Contact', 'bezlik' ) );
