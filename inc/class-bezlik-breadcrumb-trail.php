...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the Bezlik_Breadcrumb_Trail class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to Bezlik_Breadcrumb_Trail.
 * @return void
 */
function bezlik_breadcrumb_trail( $args = array() ) {
	if ( is_front_page() ) {
		return;
	}

	if ( class_exists( 'Woocommerce' ) && is_product() ) {
		echo '<div class="bezlik-breadcrumb-trail"><div class="container">';
	...
	$display = get_theme_mod('bezlik_breadcrumbs_display', array( 'is_home', 'is_single', 'is_page', 'is_archive', 'is_search' ));
	...
	if ( is_home() && in_array( 'is_home', $display ) 
		...
		$breadcrumb = apply_filters( 'bezlik_breadcrumb_trail_object', null, $args );

		if ( ! is_object( $breadcrumb ) )
			$breadcrumb = new Bezlik_Breadcrumb_Trail( $args );
	
		return $breadcrumb->trail();
	}

}
add_action( 'bezlik_after_header', 'bezlik_breadcrumb_trail', -99 );
...
class Bezlik_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `Bezlik_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see Bezlik_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Bezlik_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'bezlik_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				// Build the list item.
				$breadcrumb .= sprintf( '<%1$s %2$s>%3$s%4$s' . bezlik_get_svg_icon( $separator, false ) . '</%1$s>', tag_escape( $this->args['item_tag'] ),$attributes, $item, $meta );
			}
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="bezlik-breadcrumb-trail" itemprop="breadcrumb"><div class="container">%3$s%4$s%5$s</div></%1$s>',
		...
		$breadcrumb = apply_filters( 'bezlik_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'bezlik' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'bezlik' ),
			'home'                => esc_html__( 'Home',                                  'bezlik' ),
			'error_404'           => esc_html__( '404 Not Found',                         'bezlik' ),
			'archives'            => esc_html__( 'Archives',                              'bezlik' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'bezlik' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'bezlik' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'bezlik' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'bezlik' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'bezlik' ),
		...
		$this->labels = apply_filters( 'bezlik_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'bezlik_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'bezlik_breadcrumb_trail_items', $this->items, $this->args ) );
	}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'bezlik' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'bezlik' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'bezlik' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'bezlik' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'bezlik' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'bezlik' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'bezlik' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'bezlik' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'bezlik' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'bezlik' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'bezlik' ) ) ) );
