...
if ( !class_exists( 'Bezlik_Hero' ) ) :
	...
	Class Bezlik_Hero {
		...
		public function __construct() {		
			add_action( 'bezlik_after_header', array( $this, 'template' ) );
		}
		...
		public function header_image() {	
			$hero_before_title 	= get_theme_mod( 'hero_before_title', esc_html__( 'Hello, I\'m Elsie', 'bezlik') );
			$hero_title 		= get_theme_mod( 'hero_title', esc_html__( 'Welcome to my website', 'bezlik') );
			$button_url			= get_theme_mod( 'hero_button_url', '#' );
			$button_text 		= get_theme_mod( 'hero_button_text', esc_html__( 'Learn more', 'bezlik' ) );
			$newtab 			= '';
			...
			echo 			'<div class="hero-before-title bezlik-animated bezlik-fadeInDown">' . esc_html( $hero_before_title ) . '</div>';
			echo 			'<h2 class="hero-title bezlik-animated bezlik-fadeInDown">' . esc_html( $hero_title ) . '</h2>';
			if ( $button_url ) {
				echo		'<a ' . $newtab . ' class="button bezlik-animated bezlik-fadeInUp" href="' . esc_url( $button_url ) . '">' . esc_html( $button_text ) . '</a>';
			}
			echo 		'</div>';
			echo 	'</div>';
			echo '</div>';
				...
				<div class="element-content">
					<div class="content-inner">
						...
						<span class="entry-meta">
							<?php bezlik_post_date_author( $hero_show_author, $hero_show_date ); ?>
						</span>
					</div>
				</div>
			</div>	
						...
						<?php
							...
							while ( $query->have_posts() ) : $query->the_post(); ?>	
								...
								<?php if ( 'mixed' !== $layout ) : ?>
									...
									<?php else : ?>
										<?php $this->grid_post_markup( $hero_show_cats, $hero_show_author, $hero_show_date, 'bezlik-900x9999' ); ?>
									<?php endif; ?>
								<?php else : ?>
									<?php if ( 0 === $counter ) : ?>
										<?php $this->grid_post_markup( $hero_show_cats, $hero_show_author, $hero_show_date, 'large' ); ?>
									...
									<div class="hero-element-list">
										...
										<div class="element-content">
											<div class="content-inner">
												<?php if ( $hero_show_cats ) : ?>
												<div class="post-cats"><?php bezlik_post_cats(); ?></div>
												<?php endif; ?>
												<?php the_title( sprintf( '<h2 class="entry-title"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h2>' ); ?>
												<span class="entry-meta">
													<?php bezlik_post_date_author( $hero_show_author, $hero_show_date ); ?>
