...
bezlik.stickyHeader = {
  ...
  sticky: function sticky() {
    var header = document.getElementsByClassName('header-layout-inline')[0];
  ...
  position: function position() {
    ...
    if (document.body.classList.contains('admin-bar')) {
      header.style.top = topBar.offsetHeight + 32 + 'px';
    } else {
...
bezlik.sidebarCart = {
  init: function init() {
    if (window.jQuery) {
      ...
      jQuery('.cart-overlay, .sidebar-cart-close .bezlik-icon').on('click', function (e) {
        e.preventDefault();
        var sidebarCart = document.getElementsByClassName('sidebar-cart')[0];
        var overlay = document.getElementsByClassName('cart-overlay')[0];
        sidebarCart.classList.remove('is-open');
...
bezlikDomReady(function () {
  bezlik.navigation.init();
  bezlik.headerSearch.init();
  bezlik.stickyHeader.init();
  bezlik.sidebarCart.init();
});