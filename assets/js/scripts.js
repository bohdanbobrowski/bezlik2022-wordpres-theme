...
jQuery(document).ready(function ($) {
  ...
  $('.bezlik-devices-preview').find('button').on('click', function (event) {
    ...
    if ('desktop' == device) {
      device = 'desktop';
      var deviceControl = $(this).parents('.customize-control-responsive_number').find('.bezlik-responsive-wrapper').find('div');
      deviceControl.each(function (index) {
        if ($(this).hasClass('bezlik-preview-' + device)) {
  ...
  $(' .wp-full-overlay-footer .devices button ').on('click', function () {
    var device = $(this).attr('data-device');
    $('.bezlik-devices-preview li').removeClass('active');
    $('.bezlik-devices-preview .' + device).addClass('active');
    $('.bezlik-responsive-wrapper div').removeClass('active');
    $('.bezlik-responsive-wrapper div.bezlik-preview-' + device).addClass('active');
  });
});
/* Typography */
...
jQuery(document).ready(function ($) {
  ...
  $('.google-fonts-list').on('change', function () {
    ...
    var indexes = $.map(bodyfontcontrol.bezlikfontslist, function (obj, index) {
    ...
    $.each(bodyfontcontrol.bezlikfontslist[index].variants, function (val, text) {
    ...
    $(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.bezlikfontslist[index].category);
    bezlikGetAllSelects($(this).parent().parent().parent().parent());
  });
  $('.google_fonts_select_control select').on('change', function () {
    bezlikGetAllSelects($(this).parent().parent().parent().parent());
  });

  function bezlikGetAllSelects($element) {
    var selectedFont = {
...
jQuery(document).ready(function ($) {
  ...
  $('.bezlik-sortable_repeater_control').each(function () {
    ...
    if (numRepeaterItems > 0) {
      ...
      if (numRepeaterItems > 1) {
        ...
        for (i = 1; i < numRepeaterItems; ++i) {
          bezlikAppendRow($(this), defaultValuesArray[i]);
        }
      }
    }
  }); // Make our Repeater fields sortable

  $(this).find('.bezlik-sortable_repeater.sortable').sortable({
    update: function update(event, ui) {
      bezlikGetAllInputs($(this).parent());
    }
  }); // Remove item starting from it's parent element

  $('.bezlik-sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function (event) {
    ...
    if (numItems > 1) {
      $(this).parent().slideUp('fast', function () {
        ...
        $(this).remove();
        bezlikGetAllInputs(parentContainer);
      });
    } else {
      $(this).parent().find('.repeater-input').val('');
      bezlikGetAllInputs($(this).parent().parent().parent());
    }
  }); // Add new item

  $('.customize-control-sortable-repeater-add').click(function (event) {
    event.preventDefault();
    bezlikAppendRow($(this).parent());
    bezlikGetAllInputs($(this).parent());
  }); // Refresh our hidden field if any fields change

  $('.bezlik-sortable_repeater.sortable').change(function () {
    bezlikGetAllInputs($(this).parent());
  }); // Append a new row to our list of elements

  function bezlikAppendRow($element) {
  ...
  function bezlikGetAllInputs($element) {
...
jQuery(document).ready(function ($) {
  ...
  $.each(tabs, function (i, val) {
    $('.bezlik-customizer-tabs[data-tabs-id="' + val + '"]').find('a').on('click', function () {
      if ($(this).hasClass('tab1')) {
        $(' .tab1').addClass('active');
        $('.bezlik-customizer-tabs[data-tabs-id="' + val + '"] .tab2').removeClass('active');
      }

      if ($(this).hasClass('tab2')) {
        $('.bezlik-customizer-tabs[data-tabs-id="' + val + '"] .tab1').removeClass('active');
        $('.bezlik-customizer-tabs[data-tabs-id="' + val + '"] .tab2').addClass('active');
      }
    });
  });
});
...
jQuery(document).ready(function () {
  jQuery('.customize-control-bezlik-checkbox-multiple input[type="checkbox"]').on('change', function () {
...
jQuery(document).ready(function ($) {
  ...
  $('.customize-control-bezlik-toggle_switch').each(function () {
